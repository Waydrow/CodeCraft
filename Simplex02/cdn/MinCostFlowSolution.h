    ...
    for( int i = 0 ; i < m ; i++ ) {
        ...
        if(getArcStartNode(i)==1 && getArcEndNode(i)>nodesNum+1 && (arcAddress+i)->flow > 0) {
            ...
            for(int j=0;j<serverLevel;j++){
                ...
                if(j==serverLevel-1&&(!ok)){
                    int tmpflow=(arcAddress+i)->flow;
                    sum+=serverInfo[j][1];
                    tmpflow-=serverInfo[j][0];
                    int neibornode=myNodes[getArcEndNode(i)-nodesNum-2];
                    int siz=NodesNeighbor[ neibornode ].size();
                    for(int k=0;k<siz;k++){
                        if(tmpflow<=0)break;
                        for(int w=0;w<serverLevel;w++){
                            if(serverInfo[w][0] >= min(tmpflow,roadBanw[neibornode][k]) ){
                                sum+=serverInfo[w][1];
                                sum+=nodesMoney[ NodesNeighbor[neibornode][k] ];
                                sum+=min(tmpflow,roadBanw[neibornode][k])*roadCost[neibornode][k];
                                tmpflow-=roadBanw[neibornode][k];
                                break;
                            }
                            if(w==serverLevel-1){
                                sum+=serverInfo[w][1];
                                sum+=nodesMoney[ NodesNeighbor[neibornode][k] ];
                                sum+=serverInfo[w][0]*roadCost[neibornode][k];
                                tmpflow-=serverInfo[w][0];
                            }
                        }
                    }
