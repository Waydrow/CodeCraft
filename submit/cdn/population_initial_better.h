...
public:
    ...
    MCFSimplex *mcf;
public:
    Population(MCFSimplex *mcfs) { // constructor
        mcf = mcfs;
    ...
    void generateIntitalBetter() {
        for (int i = 0; i < POP_SCALE; i++) {
            inVec[i].bitIn = mcf->initialBetter();
        }
    }
    ...
    */
    ...
    void selectByLeague() {
        ...
        while (inVec.size() != unsigned(POP_SCALE)) {
            for (int i = 0; i < N; i++) {
                int position1 = uniform_int(0, POP_SCALE-1);
                int position2 = uniform_int(0, POP_SCALE-1);
    ...
    void mixCurrentPopulation() {
        ...
        //int randTemp = uniform_int(0, POP_SCALE - 1);
        ...
        for (int i = 0; i < maxMixNum; i++) {
            ...
            int randPostion = uniform_int(0, positionSize - 1);
            bitset<BITSIZE> bit = in.bitIn;
            bit.reset(randPostion);
            int randSearch = uniform_int(0, myEdgeNum[randPostion] - 1);
            ...
            int replacePos = uniform_int(0, POP_SCALE - 1);
