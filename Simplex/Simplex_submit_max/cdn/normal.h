...
long long mynum=0;
long long mytime=0;
int calCost(vector<int> gen,int mustCal,bool deleteExtraCost) {
    buildBasicGraph();
    int numAdd=1;
    for(int i=0; i<nodesNum; i++) {
        if(gen[i]) {
            setCapacity(S,i+numAdd,serverInfo[ gen[i]-1 ][0]);
        } else {
